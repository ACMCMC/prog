...
int main(int argc, char** argv) {
    ...
    if(operacion=='s'||operacion=='S'||operacion=='r'||operacion=='R'||operacion=='p'||operacion=='P'||operacion=='m'||operacion=='M'){
        ...
        if(valor1[0]!='-' && valor1[0]!='+'){ //Se o usuario non introduciu signo, o programa introduce un +
            for(int i=lonx-2;i>0;i--) valor1[i]=valor1[i-1];
            valor1[0]='+';
        }
        if(limpar==1){
        ...
        if(valor2[0]!='-' && valor2[0]!='+'){ //Se o usuario non introduciu signo, o programa introduce un +
            for(int i=lonx-2;i>0;i--) valor2[i]=valor2[i-1];
            valor2[0]='+';
        }
        if(limpar==1){
            while(getchar()!='\n'); //Limpamos o buffer se queda algún caracter
        ...
        if(valor3[0]!='-' && valor3[0]!='+'){ //Se o usuario non introduciu signo, o programa introduce un +
            for(int i=lonx-2;i>0;i--) valor3[i]=valor3[i-1];
            valor3[0]='+';
        }
        if(limpar==1){
            while(getchar()!='\n'); //Limpamos o buffer se queda algún caracter
    ...
    switch(operacion){
        case 'S':
        case 's': result = add(num1,num2); break;
        case 'R':
        case 'r': result = resta(num1,num2); break;
        case 'p':
        case 'P': result = mult(num1,num2); break;
        case 'M':
        case 'm': result = modulo(num1,num2); break;
        case 'F':
        case 'f': result = fact(num1); break;
        case 'D':
        case'd': result = dividir(num1,num2,num3); break;
        default:
    ...
    for (int i=result.tam-1;i>=0;i--) printf("%d",result.val[i]);
    return (EXIT_SUCCESS);
}
